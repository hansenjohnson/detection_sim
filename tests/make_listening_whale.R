## make_listening_whale ##
# functions for whale with detection capabilities
# this is like a glider that detects calls except it moves like a whale
...
whale_listening = function(
  # simulate whale movement with vectorized correlated random walk
  hrs = 24,       # number of hours
  dt = 2.5,       # time resolution [sec]
  x0 = 0,         # initial x position in m
  y0 = 0,         # initial y position in m
  bh = 'socializing',   # glider path (path1, random, linear), can add more specific ones as we go
...
# run the model for a whale listening while socializing over 24 hr period
df = whale_listening(hrs = hrs, bh = 'socializing', sub = TRUE, x0=20000, y0=-40000) %>%
  mutate(x=x/1e3,y=y/1e3,r=r/1e3)

# plot listening whale track
ggplot(df, aes(x=x,y=y))+
  geom_path()+
  coord_equal()

# plot calling whale track
ggplot()+
