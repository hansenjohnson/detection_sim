...
make_call_distribution = function(n, type = 'exponential'){
  if(type == 'exponential'){
    rexp(n = n, rate = 4.271893)  
  } else if(type == 'normal'){
    rnorm(n = n, mean = 0.2340882, sd = 0.2451971)
  } else if(type == 'logistic'){
    rlogis(n = n, location = 0.2006392, scale = 0.1286323)
  } else if(type == 'fixed'){
    rep(0.2, n)
  } else {
    warning('Call distribution not recognized! Please choose from normal, exponential, logistic, and fixed')
  }
}
...
rw_sim = function(
  ...
  call_dist_type = 'exponential', # choose call distribution
...
{ 
  ...
  cr_hr = make_call_distribution(n = nrow(df), type = call_dist_type)
...
rw_sims = function(nrws = 1e2,          # number of whales in simulation
  ...
  if(run_parallel){
    ...
    DF = mclapply(X = nseq, FUN = function(i){
      rw_sim(x0=ini$x[i],y0=ini$y[i],hrs=hrs,bh=bh,dt=dt,call_dist_type=call_dist_type)
    }, mc.cores = numCores)
