...
dfo = run_box_surveys(height = dfo_height, width = dfo_width, nrws = n_whales, n_surveys = n_surveys) %>%
  mutate(box_type = 'DFO')
tc = run_box_surveys(height = tc_height, width = tc_width, nrws = n_whales, n_surveys = n_surveys) %>%
  mutate(box_type = 'TC')
